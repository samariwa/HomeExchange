...
class Home{
   ...
   {
      return "SELECT subquery.home_id as home_id, subquery.name as name, subquery.county as county, subquery.subcounty as subcounty, subquery.average_rating as average_rating, subquery.home_tier as home_tier, subquery.home_image as home_image, subquery.availability_start_date,subquery.availability_end_date,subquery.home_availability_status FROM (SELECT homes.id as home_id, name, county, subcounty, homes.average_rating as average_rating, home_tier,home_image,availability_start_date,availability_end_date,home_availability_status, swimming_pool, wifi, tv, ac, private_gym, home_workers,security_guard,private_garden FROM homes INNER JOIN home_availability ON home_availability.home_id = homes.id INNER JOIN subcounties ON homes.address = subcounties.id INNER JOIN counties ON subcounties.county_id = counties.id INNER JOIN home_features ON homes.id = home_features.home_id WHERE home_status = '1' AND home_availability_status = '1' AND capacity >= '$capacity' AND wheelchair_accessibility >= '$wheelchair' AND kids_friendly >= '$kids' AND pets_allowed >= '$pets' AND smokers_allowed >= '$smokers' AND parking >= '$parking' AND county = '$county' AND availability_end_date >= '$enddate') subquery WHERE subquery.swimming_pool = '$swimming_pool' OR subquery.wifi = '$wifi' OR subquery.tv = '$tv1'OR subquery.ac = '$ac' OR subquery.private_gym = '$gym1' OR subquery.home_workers = '$workers' OR subquery.security_guard = '$security' OR subquery.private_garden = '$garden' OR subquery.subcounty = '$subcounty'";
   }
